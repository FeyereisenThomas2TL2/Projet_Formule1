...
void worker(int nprocesses)
{
	...
	if(getpid()==pidList[0] && nprocesses==0) //=> processus parent après tous les fork()
	{
		...
		while(smv[0]!=20)
		{
			...
			sortCars(temp, 20);
			printf("\n\n\n\n\nEssai 1 :\n\n");
			printf("\n+   Voiture |    Temps S1    |    Temps S2    |    Temps S3    |   Temps tour     |    Temps global   +\n");
			for(int i = 0; i < 20; i++){
		...
		printf("\n\nFini, vérifiez la feuille de résultats !\n");
...
int main(int argc, char *argv[])
{
	...
	int km;
	char s[100];
	puts("Bienvenue, entrez une distance de course (entre 1 et 7)!");
	do
	{		
